...
void FaceDetector::cropFace(const cv::Mat& image, cv::Mat& outCropped, cv::Mat& outSpoofness, cv::Rect& outRect) {
    cv::Rect faceRect = getLargestFace(image);
    if (faceRect.empty()) {
        return;
    }

    faceRect = faceRect & cv::Rect(0, 0, image.cols, image.rows);
    if (faceRect.empty()) {
        return;
    }
